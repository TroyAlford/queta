(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("react-glaemscribe", ["react"], factory);
	else if(typeof exports === 'object')
		exports["react-glaemscribe"] = factory(require("react"));
	else
		root["react-glaemscribe"] = factory(root["react"]);
...
var _Tengwar = __webpack_require__(1);

var _Tengwar2 = _interopRequireDefault(_Tengwar);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = _Tengwar2.default;
...
var _glaemscribe = __webpack_require__(3);

var _glaemscribe2 = _interopRequireDefault(_glaemscribe);
...
_glaemscribe2.default.resource_manager.load_modes();
_glaemscribe2.default.resource_manager.load_charsets();
if (!window.Glaemscribe) window.Glaemscribe = _glaemscribe2.default;

var modes = _glaemscribe2.default.resource_manager.loaded_modes;
var MODES = exports.MODES = Object.values(modes).map(function (mode) {
  return {
...
var Tengwar = function (_Component) {
  _inherits(Tengwar, _Component);
  ...
  function Tengwar() {
    ...
    _classCallCheck(this, Tengwar);

    for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Tengwar.__proto__ || Object.getPrototypeOf(Tengwar)).call.apply(_ref, [this].concat(args))), _this), _this.transcribe = function (text) {
      return _this.language.transcriber.transcribe(text, _this.charset);
    }, _this.transcribeChildren = function (nodes) {
      var children = Array.isArray(nodes) ? nodes : [nodes];

      return children.map(function (child, index) {
        if (typeof child === 'string') {
          var _this$transcribe = _this.transcribe(child),
              _this$transcribe2 = _slicedToArray(_this$transcribe, 2),
              success = _this$transcribe2[0],
              transcription = _this$transcribe2[1];
        ...
        if (child && child.props && child.props.children) {
          ...
          return _react2.default.createElement(
            Type,
            _extends({ key: index }, child.props),
            _this.transcribeChildren(Array.isArray(grandChildren) ? grandChildren : [grandChildren])
          );
        }
      ...
      var _this$props = _this.props,
          ...
          typeface = _this$props.typeface;

      var language = _this.language;
      var classes = ['glaemscribe', language.languageClass, language.writingClass, 'typeface-' + typeface, bold && 'bold', italic && 'italic'].filter(Boolean);

      return _react2.default.createElement(
        Tag,
        { className: classes.join(' ') },
        _this.transcribeChildren(children)
      );
    }, _temp), _possibleConstructorReturn(_this, _ret);
  }

  _createClass(Tengwar, [{
    key: 'language',
    get: function get() {
      var _this2 = this;

      return MODES.find(function (m) {
        return m.name === _this2.props.language && m.writing === _this2.props.writing;
      }) || MODES.find(function (m) {
        return m.name === _this2.props.language;
      }) || MODES[0];
    }
  }, {
    key: 'charset',
    get: function get() {
      return this.language.charsets[this.props.charset] || Object.values(this.language.charsets)[0];
    }
  }]);
...
function stringListToCleanArray(str, separator) {
  return str.split(separator).map(function (elt) {
    return elt.trim();
  }).filter(function (n) {
    return n != "";
  });;
...
Object.defineProperty(Array.prototype, "productize", {
  enumerable: false,
  value: function value(other_array) {
    var array = this;
...
Object.defineProperty(Array.prototype, "unique", {
  ...
  value: function value() {
    ...
    var uf = function uf(value, index, self) {
...
Object.defineProperty(Object.prototype, "glaem_each_reversed", {
  enumerable: false,
  value: function value(callback) {
    if (!this instanceof Array) return this.glaem_each(callback);
...
Object.defineProperty(Object.prototype, "glaem_merge", {
  enumerable: false,
  value: function value(other_object) {
    ...
    for (var o in other_object) {
      if (!other_object.hasOwnProperty(o)) continue;
...
Glaemscribe.ResourceManager.prototype.load_charsets = function (charset_list) {
  ...
  if (charset_list == null) charset_list = Object.keys(this.raw_charsets);

  // If passed a name, wrap into an array
  if (typeof charset_list === 'string' || charset_list instanceof String) charset_list = [charset_list];

  for (var i = 0; i < charset_list.length; i++) {
    var charset_name = charset_list[i];

    // Don't load a charset twice
    if (this.loaded_charsets[charset_name]) continue;

    // Cannot load a charset that does not exist
    if (!this.raw_charsets[charset_name]) continue;

    var cp = new Glaemscribe.CharsetParser();
    var charset = cp.parse(charset_name);

    if (charset) this.loaded_charsets[charset_name] = charset;
  }
};
...
Glaemscribe.ResourceManager.prototype.load_modes = function (mode_list) {
  ...
  if (mode_list == null) mode_list = Object.keys(this.raw_modes);

  // If passed a name, wrap into an array
  if (typeof mode_list === 'string' || mode_list instanceof String) mode_list = [mode_list];

  for (var i = 0; i < mode_list.length; i++) {
    var mode_name = mode_list[i];

    // Don't load a charset twice
    if (this.loaded_modes[mode_name]) continue;

    // Cannot load a charset that does not exist
    if (!this.raw_modes[mode_name]) continue;

    var mp = new Glaemscribe.ModeParser();
    var mode = mp.parse(mode_name);

    if (mode) this.loaded_modes[mode_name] = mode;
  }
};
...
Glaemscribe.VirtualChar.prototype.output = function () {
  var vc = this;
  if (vc.default) return vc.charset.n2c(vc.default).output();else return Glaemscribe.VIRTUAL_CHAR_OUTPUT;
};
...
Glaemscribe.VirtualChar.prototype.finalize = function () {
  ...
  if (vc.default) {
    ...
    if (!c) vc.charset.errors.push(new Glaemscribe.Glaeml.Error(vc.line, "Default char " + vc.default + " does not match any real character in the charset."));else if (c.is_virtual()) vc.charset.errors.push(new Glaemscribe.Glaeml.Error(vc.line, "Default char " + vc.default + " is virtual, it should be real only."));
  }
};
...
Glaemscribe.Charset.prototype.finalize = function () {
  ...
  charset.chars = charset.chars.sort(function (c1, c2) {
    if (c1.is_virtual() && c2.is_virtual()) return c1.names[0].localeCompare(c2.names[0]);
    if (c1.is_virtual()) return 1;
    if (c2.is_virtual()) return -1;
  ...
  for (var i = 0; i < charset.chars.length; i++) {
    ...
    for (var j = 0; j < c.names.length; j++) {
      ...
      if (found != null) charset.errors.push(new Glaemscribe.Glaeml.Error(c.line, "Character " + cname + " found twice."));else charset.lookup_table[cname] = c;
    }
  }
...
Glaemscribe.Glaeml.Node.prototype.is_element = function () {
  return this.type == Glaemscribe.Glaeml.NodeType.ElementInline || this.type == Glaemscribe.Glaeml.NodeType.ElementBlock;
};
...
Glaemscribe.Glaeml.Parser.prototype.parse = function (raw_data) {
    ...
    if (l[0] == "\\") {
      if (l.length == 1) {
        ...
        if (l[1] == "\\") // First backslash is escaped
          ...
          if (!parser.current_parent_node.parent_node) doc.errors.push(new Glaemscribe.Glaeml.Error(lnum, "Element 'end' unexpected."));else if (l.substring(rmatch[0].length).trim() != "") doc.errors.push(new Glaemscribe.Glaeml.Error(lnum, "Element 'end' should not have any argument."));else parser.current_parent_node = parser.current_parent_node.parent_node;
        } else {
          // Read the name of the node
          l = l.substring(1);
          rmatch = l.match(/^([a-z_]+)/);

          if (!rmatch) doc.errors.push(new Glaemscribe.Glaeml.Error(lnum, "Cannot understand element name."));else {
            var name = rmatch[0];
            var args = [];
  ...
  if (parser.current_parent_node != doc.root_node) doc.errors.push(new Glaemscribe.Glaeml.Error(lnum, "Missing 'end' element."));
...
Glaemscribe.Glaeml.Shellwords.prototype.parse = function (l) {
  ...
  for (var i = 0; i < l.length; i++) {
    ...
    if (!sw.is_eating_arg) {
      ...
      if (l[i] == "'") throw new Error("Glaeml strictly uses double quotes, not simple quotes for args");

      sw.is_eating_arg = true;
      sw.is_eating_arg_between_quotes = l[i] == "\"";

      if (!sw.is_eating_arg_between_quotes) sw.current_arg += l[i];
    } else {
...
Glaemscribe.Fragment = function (sheaf, expression) {
  ...
  if (fragment.equivalences.length == 0) fragment.equivalences = [[[""]]];
...
Glaemscribe.Fragment.prototype.is_src = function () {
  return this.sheaf.is_src();
};
Glaemscribe.Fragment.prototype.is_dst = function () {
  return this.sheaf.is_dst();
};
...
Glaemscribe.Mode.prototype.finalize = function (options) {
  ...
  options.glaem_each(function (oname, valname) {
    ...
    if (!opt) return true; // continue
    var val = opt.value_for_value_name(valname);
    if (val == null) return true; // value name is not valid
  ...
  if (mode.get_raw_mode()) mode.get_raw_mode().finalize(options);
...
Glaemscribe.Mode.prototype.get_raw_mode = function () {
  ...
  if (mode.raw_mode != null) return mode.raw_mode;

  var loaded_raw_mode = mode.raw_mode_name && Glaemscribe.resource_manager.loaded_modes[mode.raw_mode_name];
  if (loaded_raw_mode == null) return null;

  mode.raw_mode = Object.glaem_clone(loaded_raw_mode);
};

Glaemscribe.Mode.prototype.replace_specials = function (l) {
  return l.replace(/_/g, Glaemscribe.SPECIAL_CHAR_UNDERSCORE).replace(/\u00a0/g, Glaemscribe.SPECIAL_CHAR_NBSP);
};

Glaemscribe.Mode.prototype.strict_transcribe = function (content, charset, debug_context) {

  if (charset == null) charset = this.default_charset;

  if (charset == null) return [false, "*** No charset usable for transcription. Failed!"];
...
Glaemscribe.Option.prototype.default_value = function () {
  if (this.type == Glaemscribe.Option.Type.BOOL) return this.default_value_name == 'true';else return this.values[this.default_value_name];
};

Glaemscribe.Option.prototype.value_for_value_name = function (val_name) {
  if (this.type == Glaemscribe.Option.Type.BOOL) {
    if (val_name == 'true' || val_name == true) return true;

    if (val_name == 'false' || val_name == false) return false;
...
Glaemscribe.ModeParser.prototype.validate_presence_of_args = function (node, arg_count) {
  ...
  if (arg_count != null) {
    if (node.args.length != arg_count) parser.mode.errors.push(new Glaemscribe.Glaeml.Error(node.line, "Element '" + node.name + "' should have " + arg_count + " arguments."));
  }
};
...
Glaemscribe.ModeParser.prototype.validate_presence_of_children = function (parent_node, elt_name, elt_count, arg_count) {
  ...
  if (elt_count) {
    if (res.length != elt_count) parser.mode.errors.push(new Glaemscribe.Glaeml.Error(parent_node.line, "Element '" + parent_node.name + "' should have exactly " + elt_count + " children of type '" + elt_name + "'."));
  }
  if (arg_count) {
...
Glaemscribe.ModeParser.prototype.verify_mode_glaeml = function (doc) {
  ...
  doc.root_node.gpath("preprocessor.if").glaem_each(function (re, rules_element) {
    parser.validate_presence_of_args(rules_element, 1);
  });
  doc.root_node.gpath("preprocessor.elsif").glaem_each(function (re, rules_element) {
    parser.validate_presence_of_args(rules_element, 1);
  });
  doc.root_node.gpath("postprocessor.if").glaem_each(function (re, rules_element) {
    parser.validate_presence_of_args(rules_element, 1);
  });
  doc.root_node.gpath("postprocessor.elsif").glaem_each(function (re, rules_element) {
    parser.validate_presence_of_args(rules_element, 1);
  });
};
...
Glaemscribe.ModeParser.prototype.traverse_if_tree = function (root_code_block, root_element, text_procedure, element_procedure) {
  ...
  for (var c = 0; c < root_element.children.length; c++) {
    ...
    if (child.is_text()) {
      if (text_procedure != null) text_procedure(current_parent_code_block, child);
    ...
    if (child.is_element()) {
      switch (child.name) {
        ...
        default:
          ...
          if (element_procedure != null) element_procedure(current_parent_code_block, child);
  ...
  if (current_parent_code_block.parent_if_cond) mode.errors.push(new Glaemscribe.Glaeml.Error(child.line, "Unended 'if' at the end of this '" + root_element.name + "' element."));
};
...
Glaemscribe.ModeParser.prototype.parse_pre_post_processor = function (processor_element, pre_not_post) {
  ...
  var text_procedure = function text_procedure(current_parent_code_block, element) {};
  var element_procedure = function element_procedure(current_parent_code_block, element) {
    ...
    if (pre_not_post) operator_class = Glaemscribe.resource_manager.class_for_pre_processor_operator_name(operator_name);else operator_class = Glaemscribe.resource_manager.class_for_post_processor_operator_name(operator_name);
...
Glaemscribe.ModeParser.prototype.parse_raw = function (mode_name, raw, mode_options) {
  ...
  if (mode_options == null) mode_options = {};
  ...
  if (mode.errors.length > 0) return mode;
  ...
  for (var c = 0; c < charset_elements.length; c++) {
    ...
    if (charset) {
      ...
      if (is_default && is_default == "true") mode.default_charset = charset;
    } else {
  ...
  if (preprocessor_element) this.parse_pre_post_processor(preprocessor_element, true);

  // Read the postprocessor
  var postprocessor_element = doc.root_node.gpath("postprocessor")[0];
  if (postprocessor_element) this.parse_pre_post_processor(postprocessor_element, false);
  ...
  if (mode.errors.length == 0) mode.finalize(mode_options);
  ...
  for (var r = 0; r < rule_group.rules.length; r++) {
    ...
    for (var sr = 0; sr < rule.sub_rules.length; sr++) {
      ...
      for (var l = 0; l < letters.length; l++) {
        ...
        if (inchar != Glaemscribe.WORD_BREAKER && inchar != Glaemscribe.WORD_BOUNDARY) rule_group.in_charset[inchar] = rule_group;
      }
    }
  }
};
...
Glaemscribe.Sheaf = function (sheaf_chain, expression, linkable) {
  ...
  sheaf.fragment_exps = expression.split(Glaemscribe.Sheaf.SHEAF_SEPARATOR).map(function (elt) {
    return elt.trim();
  });

  if (sheaf.fragment_exps.length == 0) sheaf.fragment_exps = [""];
...
Glaemscribe.Sheaf.prototype.is_src = function () {
  return this.sheaf_chain.is_src;
};
Glaemscribe.Sheaf.prototype.is_dst = function () {
  return !this.sheaf_chain.is_src;
};
Glaemscribe.Sheaf.prototype.mode = function () {
  return this.sheaf_chain.mode();
};
...
Glaemscribe.SheafChain = function (rule, expression, is_src) {
  ...
  sheaf_chain.sheaves = sheaf_chain.sheaf_exps.map(function (sd) {
    return new Glaemscribe.Sheaf(sheaf_chain, sd['exp'], sd['linkable']);
  });

  if (sheaf_chain.sheaves.length == 0) sheaf_chain.sheaves = [new Glaemscribe.Sheaf(sheaf_chain, "", false)];
...
Glaemscribe.SheafChain.prototype.mode = function () {
...
Glaemscribe.SheafChainIterator = function (sheaf_chain, cross_schema) {
  ...
  sci.sizes = sheaf_chain.sheaves.map(function (sheaf) {
    return sheaf.fragments.length;
  });

  sci.iterators = sci.sizes.map(function (elt) {
    return 0;
  });
  ...
  if (sci.proto_attr == '') sci.proto_attr = 'CONST';

  // Construct the cross array
  if (cross_schema != null) {
    cross_schema = cross_schema.split(",").map(function (i) {
      return parseInt(i) - 1;
    });
...
Glaemscribe.Eval.Lexer.prototype.EXP_TOKENS = [new Glaemscribe.Eval.Token("bool_or", "||"), new Glaemscribe.Eval.Token("bool_and", "&&"), new Glaemscribe.Eval.Token("cond_inf_eq", "<="), new Glaemscribe.Eval.Token("cond_inf", "<"), new Glaemscribe.Eval.Token("cond_sup_eq", ">="), new Glaemscribe.Eval.Token("cond_sup", ">"), new Glaemscribe.Eval.Token("cond_eq", "=="), new Glaemscribe.Eval.Token("cond_not_eq", "!="), new Glaemscribe.Eval.Token("add_plus", "+"), new Glaemscribe.Eval.Token("add_minus", "-"), new Glaemscribe.Eval.Token("mult_times", "*"), new Glaemscribe.Eval.Token("mult_div", "/"), new Glaemscribe.Eval.Token("mult_modulo", "%"), new Glaemscribe.Eval.Token("prim_not", "!"), new Glaemscribe.Eval.Token("prim_lparen", "("), new Glaemscribe.Eval.Token("prim_rparen", ")"), new Glaemscribe.Eval.Token("prim_string", /^'[^']*'/), new Glaemscribe.Eval.Token("prim_string", /^"[^"]*"/), new Glaemscribe.Eval.Token("prim_const", /^[a-zA-Z0-9_.]+/)];
...
Glaemscribe.Eval.Parser.prototype.explore_bool = function () {
  ...
  while (!stop) {
    switch (this.lexer.advance().name) {
      case 'bool_or':
        if (v == true) this.explore_bool();else v = this.explore_compare();
        break;
      case 'bool_and':
        if (!v == true) this.explore_bool();else v = this.explore_compare();
        break;
...
Glaemscribe.Eval.Parser.prototype.explore_primary = function () {
  ...
  switch (token.name) {
    case 'prim_const':
      v = this.cast_constant(token.value);break;
    case 'add_minus':
      v = -this.explore_primary();break; // Allow the use of - as primary token for negative numbers
    case 'prim_not':
      v = !this.explore_primary();break; // Allow the use of ! for booleans
    case 'prim_lparen':

      v = this.parse_top_level();
      var rtoken = this.lexer.advance();

      if (rtoken.name != 'prim_rparen') throw "Missing right parenthesis.";
...
Glaemscribe.Eval.Parser.prototype.cast_constant = function (cst) {
  ...
  if (this.constant_is_int(cst)) return parseInt(cst);else if (this.constant_is_float(cst)) return parseFloat(cst);else if (match = cst.match(/^\'(.*)\'$/)) return match[0];else if (match = cst.match(/^\"(.*)\"$/)) return match[0];else if (cst == 'true') return true;else if (cst == 'false') return false;else if (cst == 'nil') return null;else if (this.vars[cst] != null) return this.vars[cst];else throw "Cannot understand constant '" + cst + "'.";
};
...
Glaemscribe.TranscriptionTreeNode.prototype.add_subpath = function (source, rep, path) {
  ...
  if (sibling == null) sibling = new Glaemscribe.TranscriptionTreeNode(cc, null, path_to_here);

  tree_node.siblings[cc] = sibling;

  if (source.length == 1) sibling.replacement = rep;else sibling.add_subpath(source.substring(1), rep, path_to_here);
};
...
Glaemscribe.TranscriptionTreeNode.prototype.transcribe = function (string, chain) {
  ...
  if (string != "") {
    ...
    if (sibling) return sibling.transcribe(string.substring(1), chain);
  }

  // We are at the end of the chain
  while (chain.length > 1) {
    var last_node = chain.pop();
    if (last_node.is_effective()) return [last_node.replacement, chain.length];
  }
...
Glaemscribe.TranscriptionPostProcessor.prototype.apply = function (tokens, out_charset) {
  ...
  if (this.out_space != null) {
    out_space_str = this.out_space.map(function (token) {
      return out_charset.n2c(token).output();
    }).join("");
  }
  ...
  for (var t = 0; t < tokens.length; t++) {
    ...
    switch (token) {
      ...
      default:
        ...
        if (!c) ret += Glaemscribe.UNKNOWN_CHAR_OUTPUT; // Should not happen
        else ret += c.output();
    }
  }
...
Glaemscribe.TranscriptionProcessor.prototype.finalize = function (options) {
  ...
  this.rule_groups.glaem_each(function (gname, rg) {
    rg.in_charset.glaem_each(function (char, group) {
      ...
      if (group_for_char != null) mode.errors.push(new Glaemscribe.Glaeml.Error(0, "Group " + gname + " uses input character " + char + " which is also used by group " + group_for_char.name + ". Input charsets should not intersect between groups."));else processor.in_charset[char] = group;
    });
  });
  ...
  for (var i = 0; i < chars.length; i++) {
    ...
    switch (c) {
      ...
      default:
        ...
        if (c_group == current_group) accumulated_word += c;else {
          ret = ret.concat(this.transcribe_word(accumulated_word, debug_context));
...
Glaemscribe.RxSubstitutePreProcessorOperator.prototype.finalize = function (trans_options) {
  ...
  this.finalized_glaeml_element.args[1] = this.finalized_glaeml_element.args[1].replace(/(\\\d)/g, function (cap) {
    return "$" + cap.replace("\\", "");
  });
};
...
Glaemscribe.UpDownTehtaSplitPreProcessorOperator.prototype.finalize = function (trans_options) {
  ...
  vowel_list = vowel_list.split(/,/).map(function (s) {
    return s.trim();
  });
  consonant_list = consonant_list.split(/,/).map(function (s) {
    return s.trim();
  });
...
Glaemscribe.UpDownTehtaSplitPreProcessorOperator.prototype.apply_to_word = function (w) {
  ...
  if (w.trim() == "") res.push(w);else {
    while (w.length != 0) {
      var ret = this.splitter_tree.transcribe(w);
      var r = ret[0];
      var len = ret[1];

      if (r instanceof Array && r.equals([Glaemscribe.UNKNOWN_CHAR_OUTPUT])) res.push(w[0]);else res.push(r);
...
Glaemscribe.UpDownTehtaSplitPreProcessorOperator.prototype.apply = function (content) {
  ...
  for (var li = 0; li < letters.length; li++) {
    ...
    if (this.word_split_map[letter] != null) accumulated_word += letter;else {
...
Glaemscribe.ElvishNumbersPreProcessorOperator = function (args) {
  Glaemscribe.PreProcessorOperator.call(this, args);return this;
};
...
Glaemscribe.ResolveVirtualsPostProcessorOperator.prototype.apply_loop = function (charset, tokens, new_tokens, reversed, token, idx) {
  ...
  if (c.is_virtual() && reversed == c.reversed) {
    ...
    charset.virtual_chars.glaem_each(function (_, vc) {
      var rc = vc.n2c(token);
      if (rc != null) op.last_triggers[vc.object_reference] = rc;
    });
  }
};
...
(function (global) {
  ...
  function defaults(dst, src) {
    getOwnPropertyNames(src).forEach(function (k) {
      if (!hasOwnProperty.call(dst, k)) defineProperty(dst, k, getOwnPropertyDescriptor(src, k));
    });
    return dst;
  };
  ...
  var signatureOf = function signatureOf(o) {
    return toString.call(o);
  };
  var HASWEAKMAP = function () {
    // paranoia check
    try {
      var wm = new WeakMap();
      wm.set(wm, wm);
        ...
        case '[object Object]':
          ...
          for (i = 0, l = px.length; i < l; ++i) {
            ...
            if (ck.descriptors) {
              ...
              if (ck.extensibility) {
                if (dx.writable !== dy.writable) return false;
                if (dx.configurable !== dy.configurable) return false;
              }
            }
          }
          return true;
          ...
          ck.enumerator(src).forEach(function (k) {
            ...
            if (isarray && k === 'length') {
              ...
              if (ck.descriptors) {
                ...
                if (deep && 'value' in desc) desc.value = _clone(src[k]);
  ...
  //  Install
  var obj2specs = function obj2specs(src) {
  ...
  var defaultProperties = function defaultProperties(dst, descs) {
    getOwnPropertyNames(descs).forEach(function (k) {
      if (!hasOwnProperty.call(dst, k)) defineProperty(dst, k, descs[k]);
    });
    return dst;
  };
...
//# sourceMappingURL=react-glaemscribe.js.map